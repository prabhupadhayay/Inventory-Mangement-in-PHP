...
//  src="https://rawgithub.com/trentrichardson/jQuery-Timepicker-Addon/master/jquery-ui-timepicker-addon.js"
//  src="https://rawgithub.com/trentrichardson/jQuery-Timepicker-Addon/master/jquery-ui-sliderAccess.js"
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				'<td>'+
					...
					'<input type="text" class="form-control test" name="productName[]" id="productName'+count+'" onchange="getProductData('+count+')" >'+
					'<input type="hidden" name="selectproductName" id="selectproductName'+count+'">	'+	
...
function subAmount() {
	...
	// vat
	var vat = (Number($("#subTotal").val())/100) * 1;
	vat = vat.toFixed(2);
	$("#vat").val(vat);
	$("#vatValue").val(vat);
...
function paymentOrder(orderId = null) {
	...
	//  $(document).ready(function () {
		$(".test").typeahead({
			// minlength:2,
		// $('#productTable').on('typeahead', '.test' {

		// $("#"+productNameId+"").typeahead({
            source: function (query1, result1) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
						...
						result1($.map(data.product_name, function (item1) {
							    // alert(item1);
							//   alert(index);
                               item3=item1;

							var data1= data.product_name;
							var data2=data1.indexOf(item3);
							console.log(data2);
							console.log(item1);
							return item1;
							
							// data1=data.indexof(item1);
							// alert(data1);
                        }));
                    }
                });
