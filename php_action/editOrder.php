  ...
  $paymentStatus 				= $_POST['paymentStatus'];
	...
	for($x = 0; $x < count($_POST['productName']); $x++) {		
		...
		while ($updateProductQuantityResult = $updateProductQuantityData->fetch_row()) {
			...
			$updateQuantitySql = "UPDATE product SET quantity = $editQuantity WHERE product_name = ".$_POST['productName'][$x]."";
	...
	if($readyToUpdateOrderItem) {
		...
		for($x = 0; $x < count($_POST['productName']); $x++) {			
			$updateProductQuantitySql = "SELECT product.quantity FROM product WHERE product.product_id = '$zone'";
			$updateProductQuantityData = $connect->query($updateProductQuantitySql);
			...
			while ($updateProductQuantityResult = $updateProductQuantityData->fetch_row()) {
				$updateQuantity[$x] = $updateProductQuantityResult[0] - $_POST['quantity'][$x];							
					...
					$updateProductTable = "UPDATE product SET quantity = '".$updateQuantity[$x]."' WHERE product_id = '$zone'";
					$connect->query($updateProductTable);

					// add into order_item
				$orderItemSql = "INSERT INTO order_item (order_id, product_id, quantity, rate, total, order_item_status) 
				VALUES ({$orderId}, '$zone', '".$_POST['quantity'][$x]."', '".$_POST['rateValue'][$x]."', '".$_POST['totalValue'][$x]."', 1)";
